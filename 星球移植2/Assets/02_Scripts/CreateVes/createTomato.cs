...
public class createTomato : MonoBehaviour {
	...
	void Start () {
		...
		hp = tomato.hp;	//獲得VegetableDetail_Static裡面pumpkin的生命值
		countMatureTime ();
	}
	void Awake(){
	...
	public void countMatureTime(){//計算當下階段剩餘時間
		...
		switch (growingSTATUS) {
		case GrowingSTATUS.empty:
			matureTime = plantingTime.AddSeconds(tomato.growing_00To01_time);//植物當下階段應當成熟成熟的時間(計算的值)
			break;
		case GrowingSTATUS.Growing_01:
			matureTime = plantingTime.AddSeconds(tomato.growing_01To02_time);//植物當下階段應當成熟成熟的時間(計算的值)
			break;
		case GrowingSTATUS.Growing_02:
			matureTime = plantingTime.AddSeconds(tomato.growing_02To03_time);
			break;
		case GrowingSTATUS.Growing_03:
			matureTime = plantingTime.AddSeconds(tomato.growing_03To04_time);
			break;
		}
	}

	//生成物件
	public void TomatoGrowing_01(){
		GameObject pfb = Resources.Load ("tomato/tomato_01") as GameObject;//產生Pumpkin
	...
	public void TomatoGrowing_02(){
		Destroy (transform.GetChild (2).gameObject);
		GameObject pfb = Resources.Load ("tomato/tomato_02") as GameObject;//產生Pumpkin
		GameObject prefabInstance = Instantiate (pfb);
		prefabInstance.transform.parent = this.transform;//設為子物件
		prefabInstance.transform.position = new Vector3 (this.transform.position.x, this.transform.position.y+1f, this.transform.position.z);
	}
	public void TomatoGrowing_03(){
		Destroy (transform.GetChild (2).gameObject);
		GameObject pfb = Resources.Load ("tomato/tomato_03") as GameObject;//產生Pumpkin
		GameObject prefabInstance = Instantiate (pfb);
		prefabInstance.transform.parent = this.transform;//設為子物件
		prefabInstance.transform.position = new Vector3 (this.transform.position.x, this.transform.position.y+1f, this.transform.position.z);
	}
	public void TomatoGrowing_04(){
		Destroy (transform.GetChild (2).gameObject);
		GameObject pfb = Resources.Load ("tomato/tomato_04") as GameObject;//產生Pumpkin
		GameObject prefabInstance = Instantiate (pfb);
		prefabInstance.transform.parent = this.transform;//設為子物件
		prefabInstance.transform.position = new Vector3 (this.transform.position.x, this.transform.position.y+1f, this.transform.position.z);
	}
	public void TomatoGrowing_die(){
		Destroy (transform.GetChild (2).gameObject);
		GameObject pfb = Resources.Load ("tomato/tomato_die") as GameObject;//產生Pumpkin
		GameObject prefabInstance = Instantiate (pfb);
		prefabInstance.transform.parent = this.transform;//設為子物件
		prefabInstance.transform.position = new Vector3 (this.transform.position.x, this.transform.position.y+1f, this.transform.position.z);
	}

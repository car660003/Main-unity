...
public class createEggplant : MonoBehaviour {
	...
	public void countMatureTime(){//計算當下階段剩餘時間
		...
		switch (growingSTATUS) {
		case GrowingSTATUS.empty:
			matureTime = plantingTime.AddSeconds(eggplant.growing_00To01_time);//植物當下階段應當成熟成熟的時間(計算的值)
			break;
		case GrowingSTATUS.Growing_01:
			matureTime = plantingTime.AddSeconds(eggplant.growing_01To02_time);//植物當下階段應當成熟成熟的時間(計算的值)
			break;
		case GrowingSTATUS.Growing_02:
			matureTime = plantingTime.AddSeconds(eggplant.growing_02To03_time);
			break;
		case GrowingSTATUS.Growing_03:
			matureTime = plantingTime.AddSeconds(eggplant.growing_03To04_time);
			break;
		}
	}

	//生成物件
	public void EggplantGrowing_01(){
		GameObject pfb = Resources.Load ("eggplant/eggplant_01") as GameObject;//產生Pumpkin
	...
	public void EggplantGrowing_02(){
		Destroy (transform.GetChild (2).gameObject);
		GameObject pfb = Resources.Load ("eggplant/eggplant_02") as GameObject;//產生Pumpkin
		GameObject prefabInstance = Instantiate (pfb);
		prefabInstance.transform.parent = this.transform;//設為子物件
		prefabInstance.transform.position = new Vector3 (this.transform.position.x, this.transform.position.y+1f, this.transform.position.z);
	}
	public void EggplantGrowing_03(){
		Destroy (transform.GetChild (2).gameObject);
		GameObject pfb = Resources.Load ("eggplant/eggplant_03") as GameObject;//產生Pumpkin
		GameObject prefabInstance = Instantiate (pfb);
		prefabInstance.transform.parent = this.transform;//設為子物件
		prefabInstance.transform.position = new Vector3 (this.transform.position.x, this.transform.position.y+1f, this.transform.position.z);
	}
	public void EggplantGrowing_04(){
		Destroy (transform.GetChild (2).gameObject);
		GameObject pfb = Resources.Load ("eggplant/eggplant_04") as GameObject;//產生Pumpkin
		GameObject prefabInstance = Instantiate (pfb);
		prefabInstance.transform.parent = this.transform;//設為子物件
		prefabInstance.transform.position = new Vector3 (this.transform.position.x, this.transform.position.y+1f, this.transform.position.z);
	}
	public void EggplantGrowing_die(){
		Destroy (transform.GetChild (2).gameObject);
		GameObject pfb = Resources.Load ("eggplant/eggplant_die") as GameObject;//產生Pumpkin
		GameObject prefabInstance = Instantiate (pfb);
		prefabInstance.transform.parent = this.transform;//設為子物件
		prefabInstance.transform.position = new Vector3 (this.transform.position.x, this.transform.position.y+1f, this.transform.position.z);
	}

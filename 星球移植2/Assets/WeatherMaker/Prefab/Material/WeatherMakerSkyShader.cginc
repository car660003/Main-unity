...
fixed _WeatherMakerSkySamples = 3.0;
...
procedural_sky_info CalculateScatteringColor(float3 lightDir, fixed3 lightColor, fixed sunSize, float3 eyeRay, fixed3 inScatter, fixed3 outScatter)
{
	...
	// keep alpha high as long as even a little r or g or b component
	o.skyColor.a = min(1.0, 64.0 * max(o.skyColor.r, max(o.skyColor.g, o.skyColor.b)));
...
inline float CloudShadowFBM(float2 p, float c)
{
	return CloudNoise(_FogNoise, p, c, _FogNoiseVelocity) * _FogNoiseMultiplier;
}
...
fixed ComputeCloudShadowFBMFromXZ(float2 xz)
{
	// calculate cloud values
	xz = xz * SCALE_REDUCER;
	fixed c = lerp(0.15, 0.5, _FogCover);
	fixed f = CloudShadowFBM(xz, c);
	f = saturate(1.0 - (pow(_FogSharpness, f - (1.0 - c))));

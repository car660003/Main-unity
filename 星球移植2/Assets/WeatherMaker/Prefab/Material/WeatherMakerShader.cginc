...
uniform float4 _WeatherMakerSunLightPower; // power, multiplier, shadow strength, 0
uniform float4 _WeatherMakerSunVar1; // scale, 0, 0, 0
...
uniform float4 _WeatherMakerMoonLightPower[MAX_MOON_COUNT]; // power, multiplier, shadow strength, 0
...
// these four camera variables are set for full screen effects like fog
uniform float4x4 _WeatherMakerCameraInverseMVP;
uniform float4x4 _WeatherMakerCameraInverseMV;
uniform float4x4 _WeatherMakerCameraWorldToCamera;
...
fixed4 ApplyLight(float4 lightPos, float4 lightDir, fixed4 lightColor, half4 lightAtten, float3 viewPos)
{
...
#if !defined(UNITY_COLORSPACE_GAMMA) && !defined(ORTHOGRAPHIC_MODE)
	...
	float typeMultiplier = (max(0, dot(fixed3(0, 0, 1), toLight)) * (1.0 - lightPos.w));

	// apply non-directional light
	typeMultiplier += lightPos.w;

	return lightColor * typeMultiplier * atten;
...
fixed4 ApplyLightWorldSpace(float4 lightPos, float4 lightDir, fixed4 lightColor, half4 lightAtten, float3 worldPos)
{
	float pointSpotMultiplier = min(1.0, ceil(lightPos.w));
	float3 toLight = (lightPos.xyz - (worldPos * pointSpotMultiplier));
	...
	float diff = max(pointSpotMultiplier, dot(normal, normalize(toLight)));
	return lightColor * diff * atten;

#else

	// calculate modifier for non-directional light
	float modifierNonDirectionalLight = pointSpotMultiplier;
	...
	// apply intensity to rgb for the light
...
inline fixed4 CalculateVertexColor(float3 viewPos, float ambientMultiplier)
{
	fixed3 vertexColor = UNITY_LIGHTMODEL_AMBIENT.rgb * _AmbientLightMultiplier * ambientMultiplier;
...
inline fixed3 CalculateVertexColorWorldSpace(float3 worldPos, int index)
{
	fixed3 vertexColor = fixed3(0.0, 0.0, 0.0);
	for (; index < _WeatherMakerLightCount; index++)
	{
		vertexColor += ApplyLightWorldSpace(_WeatherMakerLightPosition[index], _WeatherMakerLightSpotDirection[index], _WeatherMakerLightColor[index], _WeatherMakerLightAtten[index], worldPos);
	}
...
float3 GetFarPlaneVectorFullScreen(float4x4 inverseMVP, float4 clip)
{
...
inline bool RayBoxIntersect(float3 rayOrigin, float3 rayDir, float rayLength, float3 boxMin, float3 boxMax, out float intersectAmount, out float distanceToBox)
{
...
inline bool RaySphereIntersect(float3 rayOrigin, float3 rayDir, float rayLength, float4 spherePosition, out float intersectAmount, out float distanceToSphere)
{
...
inline bool RayPlaneIntersect(float3 rayOrigin, float3 rayDir, float3 planeNormal, float3 planePos, out float distanceToPlane)
{
...
inline void RayPlaneDiscIntersect(float3 rayOrigin, float3 rayDir, float3 planeCenter, float discRadiusSquared, inout float distanceToPlane, out bool intersectsDisc)
{
...
inline bool RayConeIntersect(float3 rayOrigin, float3 rayDir, float rayLength, float4 conePosition, float4 coneDir, float4 coneEnd, float4 coneAtten, out float intersectAmount, out float distanceToCone)
{
	float2 t;
    ...
    else
    {
    	...
    	bool hasCap1, hasCap2;

    	// case 1: ray passes down through cone disc plane
    	// handle case where the ray passes down through the cone disc plane
    	bool intersectPlane1 = RayPlaneIntersect(rayOrigin, rayDir, coneDir.xyz, coneEnd.xyz, distanceToPlane1);
    	...
    	bool intersectPlane2 = RayPlaneIntersect(rayOrigin, rayDir, -coneDir.xyz, coneEnd.xyz, distanceToPlane2);
...
inline float CalculateNoiseSphere(sampler2D noiseTex, float3 normal, float3 worldPos, float scale, float3 velocity, float multiplier)
{
	...
	float a = tex2Dlod(noiseTex, float4(worldPos.xy + a_off, 0, bias)).a;
	float b = tex2Dlod(noiseTex, float4(worldPos.xy + b_off, 0, bias)).a;
...
{
	static const float powersOfTwo[6] = { pow(2.0, 0), pow(2.0, 1), pow(2.0, 2), pow(2.0, 3), pow(2.0, 4), pow(2.0, 5) };
	static const float powersOfPoint5[6] = { pow(0.5, 0), pow(0.5, 1), pow(0.5, 2), pow(0.5, 3), pow(0.5, 4), pow(0.5, 5) };
	return
...
inline float CalculateNoiseAverage(sampler2D noiseTex, float3 normal, float3 worldPos, float scale, float2 velocity, float multiplier)
{
	...
	noiseUV += vt;
	noise = (tex2D(noiseTex, noiseUV).a - subtractor) * multiplier;// *max(normalMinMultiplier, normal.y);
	noiseUV = float2(worldPos.y, worldPos.x);
	noiseUV += vt;
	noise += (tex2D(noiseTex, noiseUV).a - subtractor) * multiplier;// *max(normalMinMultiplier, normal.z);
	noiseUV = float2(worldPos.z, worldPos.y);
	noiseUV += vt;
	noise += (tex2D(noiseTex, noiseUV).a - subtractor) * multiplier;// *max(normalMinMultiplier, normal.x);
...
inline void ApplyDither(inout fixed3 rgb, float2 screenUV, fixed3 magic, fixed l)
{
	...
	fixed gradient = frac(magic.z * frac(dot(screenUV / float2(_ScreenParams.z - 1.0, _ScreenParams.w - 1.0), magic.xy))) * l;
	//fixed gradient = frac(magic.z * frac(dot(screenUV / float2(0.00061241, 0.00033243), magic.xy))) * l;

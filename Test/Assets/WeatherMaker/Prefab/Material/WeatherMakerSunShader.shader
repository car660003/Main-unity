...
Shader "WeatherMaker/WeatherMakerSunShader"
{
	...
	SubShader
	{
		Tags { "Queue" = "Background+30" "RenderType" = "Background" "IgnoreProjector" = "True" "PerformanceChecks" = "False" }
		Cull Front Lighting Off ZWrite Off ZTest LEqual Blend OneMinusDstAlpha One
		...
		fixed4 fragBase(v2fSun i)
		{
			...
			fixed4 sunColor = fixed4(_WeatherMakerSunColor.rgb * _WeatherMakerSunTintColor.rgb * miePhase, 1.0);
			ApplyDither(sunColor.rgb, i.uv, fixed3(0.06711056, 100.00583715, 452.9829189), _WeatherMakerSkyDitherLevel);
			return sunColor;
		}
